...
from xLSTM2.xLSTM import xLSTM as xlstm

batch_size = 4
seq_lenght = 3000
input_size = 768
x_example = torch.zeros(batch_size, seq_lenght, input_size).cuda()
factor = 2 # how much input_size will be multiply to give hidden_size
depth = 1 # number of blocks for q, k and v
layers = 'm' # m for mLSTMblock and s for sLSTMblock
...
class ASDiffusionModel(nn.Module):
    def __init__(self, encoder_params, decoder_params, diffusion_params, num_classes, device):
        ...
        self.cond_types = 'zero'#diffusion_params['cond_types']
        ...
        print(encoder_params['input_dim'])
        ...
        self.encoder_original = EncoderModel(**encoder_params)
        self.decoder_original = DecoderModel(**decoder_params)

        self.xlstm = xlstm(layers, x_example, factor=factor, depth=depth).cuda()
        #self.xlstm_orin = xlstm(layers, x_example, factor=factor, depth=depth).cuda()
    ...
    def model_predictions(self, backbone_feats, x, t):
        ...
        #backbone_feats = backbone_feats #+ dct(backbone_feats)
    ...
    def get_training_loss(self, video_feats, video_feats_original, event_gt, boundary_gt,
          ...
          soft_label):
        #print(torch.sum(boundary_gt))
        if self.use_instance_norm:
            video_feats = self.ins_norm(video_feats)

        encoder_out, backbone_feats = self.encoder(video_feats, get_features=True) 
        encoder_out_orin, backbone_feats_orin = self.encoder_original(video_feats_original, get_features=True)

        #print(backbone_feats.shape) # 1,768, 3000
        '''back_feats = backbone_feats.clone()
        back_feats_orin = backbone_feats_orin.clone()'''




        backbone_feats = backbone_feats + dct(backbone_feats) #+ back_feats
        backbone_feats_orin = backbone_feats_orin + dct(backbone_feats_orin) #+ back_feats_orin

        backbone_feats, backbone_feats_orin = self.xlstm(backbone_feats.permute(0,2,1),backbone_feats_orin.permute(0,2,1))
        
        backbone_feats, backbone_feats_orin = backbone_feats.permute(0,2,1), backbone_feats_orin.permute(0,2,1)


        #backbone_feats = self.xlstm(backbone_feats.permute(0,2,1)).permute(0,2,1)
        #backbone_feats_orin = self.xlstm_orin(backbone_feats_orin.permute(0,2,1)).permute(0,2,1)

        encoder_ce_loss = encoder_ce_criterion(
            encoder_out.transpose(2, 1).contiguous().view(-1, self.num_classes), 
            event_gt.squeeze()   # batch_size must = 1
        )
        ...
        encoder_ce_loss_orin = encoder_ce_criterion(
            encoder_out_orin.transpose(2, 1).contiguous().view(-1, self.num_classes), 
            event_gt.squeeze()   # batch_size must = 1
        )

        encoder_mse_loss_orin = torch.clamp(encoder_mse_criterion(
            F.log_softmax(encoder_out_orin[:, :, 1:], dim=1), 
            F.log_softmax(encoder_out_orin.detach()[:, :, :-1], dim=1)), 
            min=0, max=16)

        encoder_boundary_loss_orin = torch.tensor(0).to(self.device) # No boundary loss for encoder
        encoder_ce_loss_orin = encoder_ce_loss_orin.mean()
        encoder_mse_loss_orin = encoder_mse_loss_orin.mean()


        ########## 

        event_diffused, noise, t = self.prepare_targets(event_gt)
        event_diffused_orin, noise_orin, t_orin = self.prepare_targets(event_gt)


        event_out = self.forward(backbone_feats, t, event_diffused, event_gt, boundary_gt)
        event_out_orin = self.forward(backbone_feats_orin, t_orin, event_diffused_orin, event_gt, boundary_gt)
        ...
        decoder_ce_loss = decoder_ce_criterion(
            ...
            event_gt.squeeze()   #torch.argmax(event_gt, dim=1).view(-1).long()   # batch_size must = 1
        )
        ...
        decoder_boundary_loss = decoder_boundary_criterion(decoder_boundary, boundary_gt.unsqueeze(0)[:,:,1:])
        ...
        decoder_boundary_orin = 1 - torch.einsum('bicl,bcjl->bijl', 
            F.softmax(event_out_orin[:,None,:,1:], 2), 
            F.softmax(event_out_orin[:,:,None,:-1].detach(), 1)
        ).squeeze(1)
        
        decoder_ce_loss_orin = decoder_ce_criterion(
            event_out_orin.transpose(2, 1).contiguous().view(-1, self.num_classes), 
            event_gt.squeeze()   #torch.argmax(event_gt, dim=1).view(-1).long()   # batch_size must = 1
        )

        decoder_mse_loss_orin = torch.clamp(decoder_mse_criterion(
            F.log_softmax(event_out_orin[:, :, 1:], dim=1), 
            F.log_softmax(event_out_orin.detach()[:, :, :-1], dim=1)), 
            min=0, max=16)
        #print(boundary_gt.shape)


        decoder_boundary_loss_orin = decoder_boundary_criterion(decoder_boundary_orin, boundary_gt.unsqueeze(0)[:,:,1:])
        decoder_boundary_loss_orin = decoder_boundary_loss.mean()

        decoder_ce_loss_orin = decoder_ce_loss_orin.mean()
        decoder_mse_loss_orin = decoder_mse_loss_orin.mean()

        loss_dict = {
            'encoder_ce_loss':2*(encoder_ce_loss+encoder_ce_loss_orin), # 2
            'encoder_mse_loss': 0*encoder_mse_loss, # 0.0
            'encoder_boundary_loss': 0.0*encoder_boundary_loss, # 0.0

            'decoder_ce_loss': 2*(decoder_ce_loss + decoder_ce_loss_orin), # 2
            'decoder_mse_loss': 0*decoder_mse_loss, #0.0
            'decoder_boundary_loss':2*(decoder_boundary_loss + decoder_boundary_loss_orin),#2
        }
    ...
    @torch.no_grad()
    def ddim_sample(self, video_feats, video_feats_orin,  seed=None):

        if self.use_instance_norm:
            video_feats = self.ins_norm(video_feats)

        encoder_out, backbone_feats = self.encoder(video_feats, get_features=True)
        encoder_out_orin, backbone_feats_orin = self.encoder_original(video_feats_orin, get_features=True)

        #backbone_feats_, backbone_feats_orin_ = self.xlstm(backbone_feats.permute(0,2,1),backbone_feats_orin.permute(0,2,1))

        #backbone_feats_, backbone_feats_orin_ = backbone_feats_.permute(0,2,1), backbone_feats_orin_.permute(0,2,1)

        backbone_feats = backbone_feats + dct(backbone_feats) #+ backbone_feats
        backbone_feats_orin = backbone_feats_orin + dct(backbone_feats_orin) #+ backbone_feats_orin
        backbone_feats, backbone_feats_orin = self.xlstm(backbone_feats.permute(0,2,1),backbone_feats_orin.permute(0,2,1))
        
        backbone_feats, backbone_feats_orin = backbone_feats.permute(0,2,1), backbone_feats_orin.permute(0,2,1)

        #ackbone_feats = self.xlstm(backbone_feats.permute(0,2,1)).permute(0,2,1)
        #backbone_feats_orin = self.xlstm_orin(backbone_feats_orin.permute(0,2,1)).permute(0,2,1)
        ...
        shape = (video_feats.shape[0], self.num_classes, video_feats.shape[1])
        ...
        x_time_orin = torch.randn(shape, device=self.device)
        ...
        for time, time_next in time_pairs:
            ...
            pred_noise_orin, x_start_orin = self.model_predictions(backbone_feats_orin, x_time_orin, time_cond)

            x_return = torch.clone(x_start)
            x_return_orin = torch.clone(x_start_orin)
            ...
            noise_orin = torch.randn_like(x_time_orin)

            x_time = x_start * alpha_next.sqrt() + \
                  c * pred_noise + \
                  sigma * noise

            x_time_orin = x_start * alpha_next.sqrt() + \
                  c * pred_noise_orin + \
                  sigma * noise_orin

        x_return = denormalize(x_return, self.scale)  
        x_return_orin = denormalize(x_return_orin, self.scale)  
        ...
        return (x_return + x_return_orin)/2
...
class MixedConvAttModuleV2(nn.Module): # for decoder
    ...
    def forward(self, x, x_cross, time_emb=None):
        ...
        #print(x.shape)
        #print(x_cross.shape)
        #if x_cross.shape[1] == 192:
        #    x_cross = x_cross.permute(0,2,1)
