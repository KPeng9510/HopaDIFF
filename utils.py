...
def edit_score(recognized, ground_truth, norm=True, bg_class=["-1"]):
...
def f_score(recognized, ground_truth, overlap, bg_class=["-1"]):
    ...
    for j in range(len(p_label)):
        intersection = np.abs(np.minimum(p_end[j], y_end) - np.maximum(p_start[j], y_start))
        
        union = np.abs(np.maximum(p_end[j], y_end) - np.minimum(p_start[j], y_start))
        judge = []
        for x in range(len(y_label)):
            ind_l = np.nonzero(y_label[x])
            #ind_l,_ = np.where((y_label[x]!=0))
...
def func_eval(label_dir, pred_dir, video_list):
    ...
    for gt_content, pred_content in tqdm(zip(video_list[0],video_list[1])):
